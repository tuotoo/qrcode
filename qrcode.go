...
func IsPositionDetectionPattern(solidGroup, hollowGroup *PosGroup) bool {
	...
	kongCenter := hollowGroup.Center
	if !(kongCenter.X > solidMinX && kongCenter.X < solidMaxX &&
		kongCenter.Y > solidMinY && kongCenter.Y < solidMaxY) {
		return false
	}
	return true
}
