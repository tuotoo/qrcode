...
func bits2bytes(datacode []bool, version int) []byte {
	...
	datacode = datacode[4+GetDataEncoder(version).CharCountBits(format) : length*8+4+GetDataEncoder(version).CharCountBits(format)]
	result := []byte{}
	for i := 0; i < length*8; {
...
func parseblock(m *Matrix, data []bool) ([]bool, []bool) {
	...
	logger.Println("version", version, "level", level)
